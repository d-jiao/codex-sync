...
- **The desktop app never shows pulled threads on its own.** Its catalog (`sqlite/codex-dev.db`) does one full build, then incremental scans that stop at an `updated_at` watermark; pulled threads are older and stay invisible. `scripts/codex-desktop-refresh.py` (app quit) re-indexes via the app's engine, copies names from `session_index.jsonl` into `threads.name`, and clears `last_full_reconciled_at` so the next launch does a full sweep. Never delete `state_5.sqlite` for this — names live only there.
