...
- **CLI layer** — `cmd/codex-sync/main.go`. Cobra commands (`init`, `push`, `pull`, `status`, `diff`, `conflicts`, `paths`, `reset`, `update`, `changelog`, `desktop`) plus Survey-driven interactive wizards; `desktop.go` holds `desktop refresh` and the `pull --desktop` hook. `paths` (`add`/`remove`/`exclude`/`unexclude`/`list`/`reset`) is backed by `internal/paths.Manager`, which edits the `sync_paths`/`exclude` fields in `config.yaml`. All user-facing output lives here.
...
- **Storage layer** — `internal/storage/`. `Storage` interface (`Upload`/`Download`/`Delete`/`DeleteBatch`/`List`/`Head`/`BucketExists`) with four adapters: `r2/`, `s3/` (also serves S3-compatible endpoints via a custom `Endpoint`), `gcs/`, `webdav/`. `ObjectInfo.Version` carries the provider's opaque revision (ETag, or the GCS generation), and the optional `ConditionalDeleter` interface adds `DeleteIfUnchanged`, which returns `storage.ErrPreconditionFailed` when the object moved on; all four adapters implement it. Adapters **self-register** via `init()` functions setting package-level `storage.NewR2` / `NewS3` / `NewGCS` / `NewWebDAV` vars; `cmd/codex-sync/main.go` blank-imports them to wire up the factory (`storage.New`). Add new providers by following this pattern.
- **Desktop layer** — `internal/desktop/`. `Refresh` makes pulled threads visible in the ChatGPT desktop app: `Backup` the databases, `IndexRollouts` (JSON-RPC to the app's own engine, `thread/list` with `useStateDbOnly=false`), `ReconcileNames` (`session_index.jsonl` → `threads.name` where empty), `ResetCatalog` (NULL `last_full_reconciled_at` for the local host in `sqlite/codex-dev.db`). Refuses with `AppRunningError` while the app or an engine is open. SQLite via `modernc.org/sqlite` (pure Go, keeps cross-compilation cgo-free; pinned to a Go 1.24-compatible release).
...
├── state.json   # per-file hash/size/mtime + remote version + last push/pull times
...
- **First pull with existing local files** is handled specially in `cmd/codex-sync/main.go` (`handleFirstPullWithExistingFiles`): shows a preview diff and offers backup-to-`~/.codex.backup.<ts>`/overwrite/abort. `createBackup` takes the configured `cfg.IsExcluded`, so the backup holds exactly what a push would upload.
...
- **The desktop app never shows pulled threads on its own.** Its catalog (`sqlite/codex-dev.db`) does one full build, then incremental scans that stop at an `updated_at` watermark; pulled threads are older and stay invisible. `codex-sync desktop refresh` / `pull --desktop` (app quit) re-index via the app's engine, copy names from `session_index.jsonl` into `threads.name`, and clear `last_full_reconciled_at` so the next launch does a full sweep. Never delete `state_5.sqlite` for this — names live only there.
...
- **Symlinks are skipped** by `GetLocalFiles` and **refused** by the pull writer — don't rely on symlinked content inside `~/.codex/` being synced, and expect a pull into a symlinked path to be reported as an error instead of followed.
- **WebDAV keys are escaped per segment** (`escapeKey`/`unescapeKey` in `internal/storage/webdav/`). Build URLs through `fullURL`/`collectionURL`; never concatenate a raw key, or a name containing `#`, `?`, `%` or a space silently addresses the wrong object.
